...
Usage: python index.py -r strong.refl
"""
import argparse
import sys
import h5py
...
def run(args):
    ...
    for t, i in zip(tables, id_values):
        if t.shape[0] < 10:
            continue
        data = t.flatten()
        rlp_ = ffs.index.ssx_xyz_to_rlp(data, wavelength, panel)
        ...
        if cell_indices is None:
            ...
            print(f"Indexed {n_indexed}/{int(data.size/3)} spots on image {i+1}")
            indexing_result = IndexingResult(
                lattices=[
            ...
            print(f"Image {i+1} results: {indexing_result.model_dump()}")
            n_indexed_images += 1
            output_crystals[identifiers_map[i]] = {"cell" : cell, "U_matrix" : orientation, "n_indexed":n_indexed}
