...
struct BG {
  ...
  */
    ...
    void forward(Matrix3d const &ori) {
        ...
        B = Matrix3d({{std::cos(phi), 0., std::sin(phi)},
                      {0., 1., 0.},
        ...
        D = Matrix3d({{std::cos(theta), std::sin(theta), 0.},
    ...
    Matrix3d back() const {
        ...
        Matrix3d Fback;
        Fback << F.a[0][0], 0.0, 0.0, F.a[0][1], F.a[1][1], 0.0, F.a[0][2], F.a[1][2],
          F.a[2][2];
...
std::vector<Matrix3d> calc_dB_dg(BG Bconverter) {
    ...
    double rad3 = g0
                  - (((g2 * g3 * g3) + (g1 * g4 * g4) - (2 * g3 * g4 * g5))
                     / ((g1 * g2) - (g5 * g5)));
    double sqrt_rad3 = sqrt(rad3);

    gradients.push_back(Matrix3d({{0.5 * trig4 / sqrt_rad3, 0, 0},
                                  {0.5 * cosp * sint / sqrt_rad3, 0, 0},
                                  {0.5 * trig2 / sqrt_rad3, 0, 0}}));

    double fac4 = (g2 * g3) - (g4 * g5);
    double rad1 = g1 - (g5 * g5 / g2);
    double rad1_three_half = sqrt(rad1 * rad1 * rad1);
    double fac3 = (g5 * g5) - (g1 * g2);
    double rad2 = -((g2 * g3 * g3) + g4 * ((g1 * g4) - (2 * g3 * g5)) + (g0 * fac3))
                  / ((g1 * g2) - (g5 * g5));
    ...
    gradients.push_back(Matrix3d(
      {{-0.5 * rat5_22 * fac1 * trig3 / rad1_three_half
          + g4 * rat5_22 * trig3 / sqrt(rad1)
          + 0.5 * fac2sq * trig4 / (fac3 * fac3 * sqrt(rad2))
       ...
       {-0.5 * rat5_22 * fac1 * cosp * cost / rad1_three_half
          + g4 * rat5_22 * cosp * cost / sqrt(rad1) + 0.5 * g4 * sinp / pow(g2, 1.5)
          + 0.5 * (fac2sq / fac3) * cosp * sint / (fac3 * sqrt(rad2)),
        0.5 * rat5_22 * (g5 * cosp * cost / sqrt(rad1) + sqrt(g2) * sinp),
        -0.5 * sinp / sqrt(g2)},
       {-0.5 * rat5_22 * fac1 * trig1 / rad1_three_half
          + g4 * rat5_22 * trig1 / sqrt(rad1)
          + 0.5 * fac2sq * trig2 / (fac3 * fac3 * sqrt(rad2))
          - 0.5 * g4 * cosf * cosp / pow(g2, 1.5),
        0.5 * rat5_22 * (g5 * trig1 / sqrt(rad1) - sqrt(g2) * cosf * cosp),
        0.5 * cosf * cosp / sqrt(g2)}}));

    gradients.push_back(Matrix3d(
      {{trig3 / sqrt(rad1) + fac4 * trig4 / (fac3 * sqrt(rad2)), 0, 0},
       {cosp * cost / sqrt(rad1) + fac4 * cosp * sint / (fac3 * sqrt(rad2)), 0, 0},
       {trig1 / sqrt(rad1) + fac4 * trig2 / (fac3 * sqrt(rad2)), 0, 0}}));

    gradients.push_back(
      Matrix3d({{-g5 * trig3 / (g2 * sqrt(rad1)) + fac2 * trig4 / (fac3 * sqrt(rad2))
                   - cosp * sinf / sqrt(g2),
                 0,
                 0},
                {-g5 * cosp * cost / (g2 * sqrt(rad1)) - sinp / sqrt(g2)
                   + fac2 * cosp * sint / (fac3 * sqrt(rad2)),
                 0,
                 0},
                {-g5 * trig1 / (g2 * sqrt(rad1)) + fac2 * trig2 / (fac3 * sqrt(rad2))
                   + cosf * cosp / sqrt(g2),
                 0,
                 0}}));
...
std::vector<double> SymmetrizeReduceEnlarge::forward_independent_parameters() {
    ...
    std::vector<double> p = {Bconverter.G.u11,
                             Bconverter.G.u22,
                             Bconverter.G.u33,
                             Bconverter.G.u12,
                             Bconverter.G.u13,
...
Matrix3d SymmetrizeReduceEnlarge::backward_orientation(
  std::vector<double> independent) {
    gemmi::SMat33<double> ustar = {independent[0],
                                   independent[1],
                                   independent[2],
                                   independent[3],
                                   independent[4],
