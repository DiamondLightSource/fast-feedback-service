...
predicted_data_rotation predict_rotation(
  Experiment<MonochromaticBeam>& experiment, const scan_varying_data& sv_data, double param_dmin, int buffer_size=0){
  ...
  if (buffer_size > 0){
    ...
    if (sv_data != scan_varying_data{}){
      throw std::runtime_error("Can't call predict function with scan varying data and an image buffer.");
    }
    std::array<int, 2> image_range = {scan.get_image_range()[0] - buffer_size, scan.get_image_range()[1] + buffer_size};
    std::array<double, 2> oscillation = {scan.get_oscillation()[0] - buffer_size * scan.get_oscillation()[1], scan.get_oscillation()[1]};
...
int main(int argc, char** argv) {
    ...
    Scan scan = expt.scan();
    if (buffer_size > 0){
      logger.info("Buffer size of {} images will be predicted either side of the scan.\n"
        "Scan static prediction is forced in this case.", buffer_size);
      param_force_static = true;
    }
    if (scan.get_oscillation()[1] == 0.0){
      //For now, only implement rotation prediction.
      logger.error("Data appears to be still shot, this program only implements rotation prediction");
      std::exit(1);
    }
    ...
    if (!param_force_static){
        ...
        if (goniometer_data.contains("setting_rotation_at_scan_points")) {
            scan_varying = true;
            r_setting_at_scan_points = goniometer_data.at("setting_rotation_at_scan_points");
    ...
    predicted_data_rotation output_data = predict_rotation(expt, sv_data, param_dmin, buffer_size);
