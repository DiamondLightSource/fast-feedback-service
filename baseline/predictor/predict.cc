...
int main(int argc, char **argv) {
    ...
    if (parser.is_used("refl")){
      ...
      logger.info("Sigma b estimate (degrees): {:.6f} on {} reflections", avg, filtered_sigma_b_data.extent(0));
      double avg2 = 180.0 * std::pow(sigma_m_total / n_sigma_m, 0.5) / M_PI;
      logger.info("Sigma m estimate (degrees): {:.6f} on {} reflections", avg2, n_sigma_m);
      ...
      for (int i=0;i<xyzcal.extent(0);++i){
        ...
        if (180.0 * std::pow(val, 0.5) / M_PI < 0.1){
          tot_rmsd += val;
          count++;
        }
        
      }
      double avg3 = 180.0 * std::pow(tot_rmsd/count, 0.5) / M_PI;
      logger.info("Sigma b deviaton (degrees): {:.6f} calculated on {} reflections", avg3, count);
