    ...
    af::versa<bool, af::c_grid<2> > gain(
      ...
      double n_sigma) {
        ...
        for (std::size_t i = 0; i < image.size(); ++i) {
            if (temp[i]) {
                FloatType bound =
                  gain[i] + n_sigma * gain[i] * std::sqrt(2.0 / (count[i] - 1));
                result[i] = (index_of_dispersion_image[i] > bound) ? 1 : 0;
            }
        }
      ...
      int min_count) {
        ...
        for (std::size_t i = 0; i < image.size(); ++i) {
            if (temp[i]) {
                ...
                result[i] =
                  (index_of_dispersion_image[i] > bnd_b && image[i] > bnd_s) ? 1 : 0;
            }
        }
    ...
    af::versa<bool, af::c_grid<2> > dispersion_w_gain(
      ...
      int min_count) {
        ...
        for (std::size_t i = 0; i < image.size(); ++i) {
            if (temp[i]) {
                FloatType bnd_b =
                  gain[i] + nsig_b * gain[i] * std::sqrt(2.0 / (count[i] - 1));
    ...
    class DispersionExtendedThresholdDebug {
      ...
      private:
        void init(const af::const_ref<double, af::c_grid<2> >& image,
            ...
            // Widen the kernel slightly and compute the mean image without strong
            // pixels
            size[0] += 2;
            size[1] += 2;
            ...
            for (std::size_t i = 0; i < image.size(); ++i) {
                if (mask[i]) {
                    ...
                    value_mask_[i] =
                      (distance[i] >= erosion_distance) && (image[i] >= bnd_s);
                    final_mask_[i] = cv_mask_[i] && value_mask_[i] && global_mask_[i];
                }
