...
TEST(BaselineIndexer, assign_indices_test) {
    ...
    std::vector<double> rlp_data = {
      -0.20806554291174043, -0.20006695964877577, 0.46900930253991086,
      -0.20711023322729, -0.1844213770514764, 0.47299170443928473,
      -0.24784778679168532, -0.10882416182889978, 0.47089025552307,
      -0.20693750699327926, -0.1837374215284657, 0.4732856446956432,
      -0.24768402495471797, -0.11030902243741092, 0.4706897967841226,
      -0.565, 0.15, 0.027 // Final point which will be outside a tolerance of 0.2
    };
    std::vector<double> xyzobs_mm_data = {
      128.09295901467863, 102.49866279545705, 2.325651228282444,
      128.5569574846125, 103.27824528734998, 2.360557813322331,
      119.93326007874127, 113.97862045047734, 2.5612706773016787,
      128.71319842438626, 317.368360910889, 4.620759194654988,
      119.97077430898872, 306.5586092785435, 4.751658888554562,
      65.0, 226.0, 0.013
    };
    int count;
    ...
    for (int i = 0; i < expected_miller_indices.size(); ++i) {
        Vector3i assigned_index = {miller_index_span(i,0), miller_index_span(i,1),miller_index_span(i,2)};
