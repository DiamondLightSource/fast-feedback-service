...
Matrix3d axis_and_angle_as_matrix(Vector3d axis, double angle, bool deg=false){
    double q0=0.0;
    double q1=0.0;
    double q2=0.0;
    double q3=0.0;
...
Goniometer::Goniometer(
    ...
    if (scan_axis_ >= axes_.size()){
        throw std::invalid_argument("Goniometer scan axis number is out of range of axis length");
    }
    init();
}

Matrix3d Goniometer::get_setting_rotation() const{
    return setting_rotation_;
}

Matrix3d Goniometer::get_sample_rotation() const {
    return sample_rotation_;
}

Vector3d Goniometer::get_rotation_axis() const {
...
Goniometer::Goniometer(json goniometer_data){
    ...
    for (const auto &key : required_keys) {
        if (goniometer_data.find(key) == goniometer_data.end()) {
            throw std::invalid_argument(
                "Key " + key + " is missing from the input goniometer JSON");
        }
    }
    std::vector<Vector3d> axes;
    for (json::iterator it=goniometer_data["axes"].begin();it != goniometer_data["axes"].end();it++){
        Vector3d axis;
        axis[0] = (*it)[0];
        axis[1] = (*it)[1];
        axis[2] = (*it)[2];
    ...
    axes_ = axes;
    std::vector<double> angles;
    for (json::iterator it=goniometer_data["angles"].begin();it != goniometer_data["angles"].end();it++){
        angles.push_back(*it);
    }
    angles_ = angles;
    std::vector<std::string> names;
    for (json::iterator it=goniometer_data["names"].begin();it != goniometer_data["names"].end();it++){
        names.push_back(*it);
    }
    names_ = names;
