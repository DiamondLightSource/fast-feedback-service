...
int main(int argc, char** argv) {
    ...
    parser.add_argument("--dmin").help("Resolution limit").scan<'f', float>();
    ...
    std::tie(grid_points_per_void, centres_of_mass_frac) =
      flood_fill(real_fft, 15.0, n_points);
    std::tie(grid_points_per_void, centres_of_mass_frac) =
      flood_fill_filter(grid_points_per_void, centres_of_mass_frac, 0.15);

    std::vector<Vector3d> candidate_vecs = sites_to_vecs(
    ...
    if (candidate_vecs.size() < 3) {
        ...
        Crystal best_xtal{
          candidate_vecs[0], candidate_vecs[1], candidate_vecs[2], space_group};
