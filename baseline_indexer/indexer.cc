...
int main(int argc, char **argv) {
    ...
    parser.add_argument("--max-cell")
      ...
      .scan<'f', float>();
    parser.add_argument("--fft-npoints") // mainly for testing, likely would always want to keep it as 256.
    ...
    std::vector<bool> used_in_indexing = fft3d(rlp, real_fft_result, d_min, b_iso, n_points);
    ...
    std::tie(grid_points_per_void, centres_of_mass_frac) = flood_fill(real_fft_result, 15.0, n_points);
    ...
    std::vector<Vector3d> candidate_lattice_vectors = sites_to_vecs(centres_of_mass_frac, grid_points_per_void, d_min, 3.0, max_cell, n_points);
    ...
    if (candidate_lattice_vectors.size() < 3){
      ...
      Crystal best_xtal {candidate_lattice_vectors[0], candidate_lattice_vectors[1], candidate_lattice_vectors[2], space_group};
