...
TEST(BaselineIndexer, flood_fill_test) {
    ...
    std::vector<int> corner_cube{{0,4,20,24,100,104,120,124}}; // cube across all 8 corners
    // i.e. at fractional coords (0,0,0), (0,0.8,0), (0,0,0.8), ... (0.8,0.8,0.8)
    std::vector<int> channel{{12,37,38,39,42,43,62,63,67,112}}; // a channel with a break
    ...
    std::tie(grid_points_per_void, centres_of_mass_frac) = flood_fill(grid, 0.0001, n_points);
...
TEST(BaselineIndexer, flood_fill_filter_test) {
    ...
    std::tie(grid_points_per_void_out, centres_of_mass_frac_out) = flood_fill_filter(
        grid_points_per_void, centres_of_mass_frac, 0.2);
    ...
    for (int i=0;i<grid_points_per_void_out.size();i++){
        ...
        for (int j=0;j<3;j++){
            EXPECT_DOUBLE_EQ(centres_of_mass_frac_out[i][j], expected_centres_of_mass[i][j]);
        }
    }
}