...
TEST(BaselineIndexer, peaks_to_rlvs_test) {
    ...
    grid_points_per_void.push_back(10);
    centres_of_mass_frac.push_back({0.75,0.75,0.75}); // rlp = (64.0, 64.0, 64.0) = 110.85A
    centres_of_mass_frac.push_back({0.1,0.1,0.1}); // rlp = (25.6, 25.6,25.6) = 44.34A
    centres_of_mass_frac.push_back({0.4,0.4,0.4}); // rlp = (102.4, 102.4,102.4) = 177.36A
    double dmin = 2.0;
    double min_cell = 3.0;
    ...
    centres_of_mass_frac[2] = {0.405,0.405,0.405}; // similar but not exactly equal to index 1
    grid_points_per_void[1] = 10; // i.e. wouldn't get filtered out by approx multiple filter
    std::vector<Vector3d> unique3 = peaks_to_rlvs(
        centres_of_mass_frac, grid_points_per_void, dmin, min_cell, max_cell, n_points
    );
    EXPECT_EQ(unique3.size(), 2);
    for (int i=0;i<3;i++){
        EXPECT_DOUBLE_EQ(unique3[0][i], -103.04); // mean of point at 0.6,0.6,0.6 and 0.405,0.405,0.405
    }
    for (int i=0;i<3;i++){
