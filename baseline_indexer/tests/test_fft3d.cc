...
TEST(BaselineIndexer, map_centroids_to_reciprocal_space_test) {
    ...
    EXPECT_DOUBLE_EQ(std::accumulate(complex_data_in.begin(), complex_data_in.end(), std::complex{0.0,0.0}).real(), 2.0);
    
    // Now set a biso;
    double b_iso_2=10.0;
    std::vector<std::complex<double>> complex_data_in_2(n_points * n_points * n_points);
    map_centroids_to_reciprocal_space_grid(reciprocal_space_vectors, complex_data_in_2, used_in_indexing, d_min, b_iso_2, n_points);
    ...
    //check no other values have been written
    EXPECT_DOUBLE_EQ(std::accumulate(complex_data_in_2.begin(), complex_data_in_2.end(), std::complex{0.0,0.0}).real(), 1.5610055003931969);
    ...
    map_centroids_to_reciprocal_space_grid(reciprocal_space_vectors, complex_data_in_3, used_in_indexing, dmin_2, b_iso_2, n_points);
    ...
    // check no other values have been written
    EXPECT_DOUBLE_EQ(std::accumulate(complex_data_in_3.begin(), complex_data_in_3.end(), std::complex{0.0,0.0}).real(), 0.86070797642505781);
}