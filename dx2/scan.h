...
class Scan {
// A class to represent the physical measurement, consisting of the number of images,
// starting oscillation and a constant oscillation width between sequential images.
// This class MUST NOT be modified during processing or used to track additional metadata.
public:
    Scan()=default;
...
Scan::Scan(json scan_data){
    ...
    std::vector<std::string> required_keys = {
        "image_range", "properties"
    };
    for (const auto &key : required_keys) {
        if (scan_data.find(key) == scan_data.end()) {
            throw std::invalid_argument(
                "Key " + key + " is missing from the input scan JSON");
        }
    }
    image_range_ = scan_data["image_range"];
    num_images_ = image_range_[1] - image_range_[0]+1;
    if (scan_data["properties"].find("oscillation") == scan_data["properties"].end()){
        throw std::invalid_argument(
    ...
    for (json::iterator it = osc_array.begin(); it != osc_array.begin()+2; ++it) {
...
json Scan::to_json() const {
    ...
    scan_data["batch_offset"] = 0; // We MUST NOT use batch offsets in dx2, written out here for compatibility
    ...
    //FIXME for dials compatibility, do we need exposure times and epochs arrays, also valid_image_ranges empty dict?
    return scan_data;
}

std::array<int, 2> Scan::get_image_range() const {
    return image_range_;
}
