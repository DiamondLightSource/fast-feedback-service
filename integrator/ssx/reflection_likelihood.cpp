...
DerivativeMatrices rotate_mat3_double(const Eigen::Matrix3d &R,
...
DerivativeMatrices compute_dS(const Simple6MosaicityParameterisation &model,
                              const Eigen::Matrix3d &R) {
...
ConditionalDistribution::ConditionalDistribution(double norm_s0,
                                                 const Vector3d &mu,
                                                 const Matrix3d &S,
...
const SigmaDerivativeMatrices &ConditionalDistribution::first_derivatives_of_sigma()
  const {
    return dSbar_;
}

const MuDerivativeVectors &ConditionalDistribution::first_derivatives_of_mean() const {
    return dmbar_;
}
