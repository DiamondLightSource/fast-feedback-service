...
class FisherScoringMaximumLikelihood {
  public:
    FisherScoringMaximumLikelihood(Simple6MosaicityParameterisation &model,
  ...
  private:
    double log_likelihood(const ParameterVector &x);

    ParameterVector score(const ParameterVector &x);

    std::pair<ParameterVector, FisherMatrix> score_and_fisher_information(
      const ParameterVector &x);

    ParameterVector solve_update_equation(const ParameterVector &S,
                                          const FisherMatrix &I) const;

    double line_search(const ParameterVector &x,
                       const ParameterVector &p,
                       double tau = 0.5,
                       double delta = 1.0);

    ParameterVector gradient_search(const ParameterVector &x);

    bool test_LL_convergence() const;

    void callback(const ParameterVector &x);
