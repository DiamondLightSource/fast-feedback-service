  ...
  for (size_t _slow = 0; _slow < slow; _slow++) {
    ...
    for (size_t _fast = 0; _fast < fast; _fast++) {
      for (size_t row = 0; row < E2XE_MOD_SLOW; row++) {
        size_t offset = (row0 + row * image->fast +
                         _fast * (E2XE_MOD_FAST + E2XE_GAP_FAST));
        size_t target =
            (_slow * fast + _fast) * module_pixels + row * E2XE_MOD_FAST;
        memcpy((void*)&modules->data[target], (void*)&image->data[offset],
  ...
  for (; data_file < obj->data_file_count; data_file++) {
    if ((subtracted + obj->data_files[data_file].offset) <
        (obj->data_files[data_file].frames +
         obj->data_files[data_file].offset)) {
      break;
    } else {
...
h5_data_file* get_data_file(h5read_handle* obj, size_t index) {
  ...
  if (current->file == 0) {
    if (access(current->filename, F_OK) == 0) {
      ...
      hid_t file = H5Fopen(current->filename,
                           H5F_ACC_RDONLY | H5F_ACC_SWMR_READ, H5P_DEFAULT);
  ...
  if (current->file != 0 && current->dataset == 0) {
    ...
    if (dataset < 0) {
      fprintf(stderr, "Error: Reading datasets of child file %s\n",
              current->filename);
      exit(1);
    }
    ...
    if (H5Sget_simple_extent_ndims(space) != 3) {
      fprintf(stderr, "Error: Data file %s data are not three dimensional\n",
              current->filename);
      exit(1);
    }
    hsize_t dims[3];
    ...
    if (dims[0] != current->frames) {
      fprintf(
          ...
          current->filename, dims[0], current->frames);
      exit(1);
    }
    if (dims[1] != obj->slow) {
      fprintf(stderr,
              ...
              "expected %ld\n",
              current->filename, dims[1], obj->slow);
      exit(1);
    }
    if (dims[2] != obj->fast) {
      fprintf(stderr,
              ...
              "expected %ld\n",
              current->filename, dims[2], obj->fast);
      exit(1);
    }
  }
  return current;
...
void h5read_get_raw_chunk(h5read_handle* obj, size_t index, size_t* size,
  ...
  *size = chunk_size;
  ...
  uint32_t filter = 0;
  herr_t err =
      H5Dread_chunk(current->dataset, H5P_DEFAULT, offset, &filter, data);
  if (err < 0) {
...
void h5read_get_image_into(h5read_handle* obj, size_t index,
                           image_t_type* data) {
  if (index >= obj->frames) {
    fprintf(stderr, "Error: image %ld greater than number of frames (%ld)\n",
            index, obj->frames);
    exit(1);
  }
  ...
  if (H5Dread(current->dataset, datatype, mem_space, space, H5P_DEFAULT, data) <
      0) {
...
void h5read_get_trusted_range(h5read_handle* obj, image_t_type* min,
                              image_t_type* max) {
...
void read_mask(h5read_handle* obj) {
  ...
  obj->module_mask =
      (uint8_t*)malloc(sizeof(uint8_t) * fast * slow * module_pixels);
  for (size_t _slow = 0; _slow < slow; _slow++) {
    ...
    for (size_t _fast = 0; _fast < fast; _fast++) {
      for (size_t row = 0; row < E2XE_MOD_SLOW; row++) {
        ...
        memcpy((void*)&obj->module_mask[target], (void*)&obj->mask[offset],
...
herr_t _read_single_value_image_t_type(hid_t origin, const char* dataset_path,
  ...
  if (datatype_size < sizeof(image_t_type)) {
    ...
    fprintf(stderr,
            ...
            "type\n",
            name, dataset_path, datatype_size);
    exit(1);
  }
#ifdef PIXEL_DATA_32BIT
  if (H5Dread(dataset, H5T_NATIVE_UINT32, H5S_ALL, H5S_ALL, H5P_DEFAULT,
              destination) < 0) {
#else
  if (H5Dread(dataset, H5T_NATIVE_UINT16, H5S_ALL, H5S_ALL, H5P_DEFAULT,
              destination) < 0) {
#endif
    char name[256] = "\0";
    H5Iget_name(origin, name, 256);
...
herr_t _read_single_value_float(hid_t origin, const char* path,
                                float* destination) {
  ...
  if (num_elements > 1) {
    ...
    *destination = -1;
    ...
    fprintf(stderr,
            ...
            "data type\n",
            name, path, datatype_size);
    *destination = -1;
  } else if (H5Dread(dataset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                     destination) < 0) {
    // Get the name of the origin for output
    char name[256] = "\0";
    H5Iget_name(origin, name, 256);
...
void read_trusted_range(h5read_handle* obj) {
  ...
  if (_read_single_value_image_t_type(
          obj->master_file, "/entry/instrument/detector/saturation_value",
          &obj->trusted_range_max) < 0) {
    fprintf(stderr,
            "Warning: No saturation_value found, using maximum value\n");
  }

  // Try to read underload value, but don't fail if it doesn't exist
  if (_read_single_value_image_t_type(
          obj->master_file, "/entry/instrument/detector/underload_value",
          &obj->trusted_range_min) < 0) {
...
void read_oscillation_start_and_width(h5read_handle* obj) {
  ...
  if (H5Dread(omega_dataset, datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT, buffer) <
      0) {
...
int vds_info(char* root, hid_t master, hid_t dataset,
             h5_data_file** data_files_array) {
...
int unpack_vds(const char* filename, h5_data_file** data_files) {
  ...
  // allow for that here
  hid_t file =
      H5Fopen(filename, H5F_ACC_RDONLY | H5F_ACC_SWMR_READ, H5P_DEFAULT);
...
void setup_data(h5read_handle* obj) {
  hid_t vds_dataset =
      H5Dopen2(obj->master_file, "/entry/data/data", H5P_DEFAULT);
...
uint8_t* _generate_e2xe_16m_mask() {
  assert(E2XE_MOD_SLOW * E2XE_16M_NSLOW +
             E2XE_GAP_SLOW * (E2XE_16M_NSLOW - 1) ==
         E2XE_16M_SLOW);
  assert(E2XE_MOD_FAST * E2XE_16M_NFAST +
             E2XE_GAP_FAST * (E2XE_16M_NFAST - 1) ==
         E2XE_16M_FAST);
...
#endif
  const char* USAGE =
      implicit_sample ? "Usage: %s [-h|--help] [-v] [FILE.nxs | --sample]\n"
...
#else
    fprintf(stderr,
            "Error: Cannot read nexus file as h5read built without HDF5\n");
#endif
  }
  if (handle == NULL) {
