...
int main(int argc, char **argv) {
  ...
  image_t_type *buffer =
      malloc(h5read_get_image_fast(obj) * h5read_get_image_slow(obj) *
             sizeof(image_t_type));
  image_t_type *buffer_manual =
      malloc(h5read_get_image_fast(obj) * h5read_get_image_slow(obj) *
             sizeof(image_t_type));

  size_t max_compressed_bytes = h5read_get_image_fast(obj) *
                                h5read_get_image_slow(obj) *
                                sizeof(image_t_type);
