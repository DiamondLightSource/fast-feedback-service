...
std::vector<std::size_t> read_flags_data(string filename, string array_name){
    auto start_time = std::chrono::high_resolution_clock::now();
    hid_t file = H5Fopen(filename.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);
    if (file < 0){
      std::cout << "Error: Unable to open " << filename.c_str() << " as a hdf5 reflection table" <<std::endl;
    ...
    size_t num_elements = H5Sget_simple_extent_npoints(dataspace);
    hid_t space = H5Dget_space(dataset);
    std::vector<std::size_t> data_out(num_elements);
    ...
    float total_time =
      std::chrono::duration_cast<std::chrono::duration<double>>(
        std::chrono::high_resolution_clock::now() - start_time)
        .count();
    std::cout << "phi READ TIME " << total_time << "s" << std::endl;
