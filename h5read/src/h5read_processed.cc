...
template <typename T>
std::vector<T> read_array_from_h5_file(std::string filename, std::string array_name){
    ...
    float total_time =
      std::chrono::duration_cast<std::chrono::duration<double>>(
        ...
        .count();
    std::cout << "READ TIME for " << array_name << " : " << total_time << "s" << std::endl;
...
template std::vector<double> read_array_from_h5_file(std::string, std::string);
template std::vector<std::size_t> read_array_from_h5_file(std::string, std::string);
