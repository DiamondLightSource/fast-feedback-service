 ...
 */
__host__ __device__ inline auto span(Vector3D& v)
  -> std::experimental::mdspan<scalar_t, std::experimental::extents<size_t, 3>> {
    return std::experimental::mdspan<scalar_t, std::experimental::extents<size_t, 3>>(
      &v.x);
}
 ...
 */
__host__ __device__ inline auto span(const Vector3D& v)
  -> std::experimental::mdspan<const scalar_t, std::experimental::extents<size_t, 3>> {
    return std::experimental::mdspan<const scalar_t,
                                     std::experimental::extents<size_t, 3>>(&v.x);
}
}  // namespace fastvec