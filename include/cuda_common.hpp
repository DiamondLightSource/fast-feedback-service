...
class DeviceBuffer {
   ...
   */
    DeviceBuffer(size_t count) : count_(count) {
        ...
        logger.debug("Allocated {} bytes of device memory for {} elements",
                     count * sizeof(T),
                     count);
    }
    ...
    ~DeviceBuffer() {
        if (device_ptr_) {
            ...
            if (err != cudaSuccess) {
                ...
                logger.debug("Freed device memory for {} elements", count_);
            }
        }
    }
   ...
   */
    void assign(const T *host_data) {
        ...
        logger.debug("Copied {} bytes from host to device", count_ * sizeof(T));
    }
   ...
   */
    void extract(T *host_data) const {
        ...
        logger.debug("Copied {} bytes from device to host", count_ * sizeof(T));
    }
