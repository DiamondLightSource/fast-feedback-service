  ...
  float n_sig_s) {
    ...
    for (int i = -kernel_radius; i <= kernel_radius; ++i) {
        for (int j = -kernel_radius; j <= kernel_radius; ++j) {
            // Calculate the local coordinates, clamp to the image bounds
            int lx = local_x + j;
            int ly = local_y + i;
...
__global__ void dispersion_extended_second_pass(
  ...
  size_t dispersion_mask_pitch) {
    ...
    int row_start = max(0, local_y - KERNEL_RADIUS_EXTENDED);
    int row_end = min(local_y + KERNEL_RADIUS_EXTENDED + 1, kernel_constants.height);

    for (int row = row_start; row < row_end; ++row) {
        int col_start = max(0, local_x - KERNEL_RADIUS_EXTENDED);
        int col_end = min(local_x + KERNEL_RADIUS_EXTENDED + 1, kernel_constants.width);

        for (int col = col_start; col < col_end; ++col) {
            pixel_t pixel = shared_image(col, row);
            uint8_t mask_pixel = shared_mask(col, row);
            uint8_t disp_mask_pixel = shared_dispersion_mask(col, row);
            /*
