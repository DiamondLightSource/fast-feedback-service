...
ConnectedComponents::ConnectedComponents(const uint8_t *result_image,
    ...
    // Build graph
    auto index_to_vertex_map = build_graph(width, height);

    // Generate bounding boxes
    generate_boxes(index_to_vertex_map, width, height, min_spot_size);
}
 ...
 */
std::unordered_map<size_t, size_t> ConnectedComponents::build_graph(
  const ushort width,
  const ushort height) {
    ...
    for (const auto &[linear_index, signal] : signals) {
        /*
         * We build two maps here:
         *
         * - index_to_vertex is a local map that is used to map linear indices to
         * graph vertex IDs. This is used to efficiently add edges to the graph
         * in 2d connected components analysis. Therefore, this map is not stored
         * as a member variable as it is only used during 2d graph construction.
         * 
         * - vertex_to_index is stored as a member variable and is used to map
         * graph vertex IDs to linear indices. This is utilised in 3d connected
         * components analysis to relate graph vertices back to the original pixels
         * when building the 3D graph. Thus is it stored for later.
         */
 ...
 */
void ConnectedComponents::generate_boxes(
  const std::unordered_map<size_t, size_t> &index_to_vertex,
  const ushort width,
  const ushort height,
  const uint32_t min_spot_size) {
